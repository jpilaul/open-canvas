...
```bash
# The root `.env` file will be read by Temporal for the agents.
cp .env.example .env
```
...
After Temporal is running, execute the following command inside `apps/web` to start the Open Canvas frontend:
